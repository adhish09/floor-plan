...
const mtl = require('./iphone.mtl');
const obj = require('./iphone.obj');
const img = require('./camera-lens-icon.jpg');

const width = {length: 60, unit: 'cm'};
const depth = {length: 30, unit: 'cm'};
const height = {length: 20, unit: 'cm'};

let cachedJSONiphone = null;
...
export default {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONiphone) {
      ...
      let object = loader.parse(cachedJSONiphone);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img) + '/')
      .then(object => {
        cachedJSONiphone = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONiphone))
      });
  },
