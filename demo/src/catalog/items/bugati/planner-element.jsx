...
const mtl = require('./bugatti.mtl');
const obj = require('./bugatti.obj');
const img = require('./bugatti.png');

const width = {length: 30, unit: 'cm'};
const depth = {length: 20, unit: 'cm'};
const height = {length: 10, unit: 'cm'};

let cachedJSONbugatti = null;
...
export default {
  ...
  render3D: function (element, layer, scene) {
    ...
    if (cachedJSONbugatti) {
      let loader = new ObjectLoader();
      let object = loader.parse(cachedJSONbugatti);
      return Promise.resolve(onLoadItem(object));
    }

    return loadObjWithMaterial(mtl, obj, path.dirname(img) + '/')
      .then(object => {
        cachedJSONbugatti = object.toJSON();
        let loader = new ObjectLoader();
        return onLoadItem(loader.parse(cachedJSONbugatti))
      });
  },
